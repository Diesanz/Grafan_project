...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Práctica1_ Grafana/ESI-Grupo6-DB.drawio (3).png}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{vsz}: Tamaño virtual del proceso en KB.
        \item \textbf{rss}: Tamaño residente del proceso en KB.
        ...
        \item \textbf{estado}: Estado del proceso (ej. "ejecutando", "dormido").
    \end{itemize}
    \item \textbf{Num\_procesos:} Tabla para almacenar el número total de procesos en ejecución.
    \begin{itemize}
        \item \textbf{timestamp}: Marca de tiempo de la medición.
        \item \textbf{operaciones}: Número total de procesos.
    \end{itemize}
...
def get_connection():
    try:
        ...
        return: Devuelve un objeto de tipo conexión a la base
        de datos si la conexión es exitosa.
...
El procedimiento acepta varios parámetros de entrada (IN) que representan los datos de rendimiento del servidor (excepto los valores correspondientes a las tablas interfaces y procesos).\\

Una vez recibidos estos parámetros, 'AddDatos' calcula la marca de tiempo actual que registrará en qué momento se han recopilado los datos. Además, define un manejador de errores que se activa si ocurre alguna excepción SQL durante la ejecución del procedimiento.\\

Posteriormente, comienza una transacción para asegurar que todas las operaciones de inserción se realicen con éxito o se reviertan en caso de error.\\
...
\begin{enumerate}
    ...
    \begin{lstlisting}[language=python]
    interfaces_info = []
    for nombre_interfaz, interface_addreses in
            ...
            for address in interface_addreses:
                ...
                'AddressFamily.AF_INET':
                    direccion_ipv4 = address.address 
                        if address.address else "IPv4"
                    subnet = address.netmask 
                        if address.netmask else "N/A"
                    broadcast = address.broadcast 
                        if address.broadcast else "N/A"
                elif str(address.family) ==
                'AddressFamily.AF_PACKET':
                    direccion_mac = address.address 
                        if address.address else "MAC"
            bytes_enviados = net_io[nombre_interfaz].bytes_sent 
                if nombre_interfaz in net_io else 0
            bytes_recibidos = net_io[nombre_interfaz].bytes_recv
                if nombre_interfaz in net_io else 0
...
\begin{lstlisting}[language=SQL]
    SELECT nombre as Nombre, direccion as Dirección,
        mascara as Máscara, mac as Mac 
    FROM grafana.interfaces 
\end{lstlisting}
...
\begin{lstlisting}[language=SQL]
    SELECT carga1 as 1minuto, carga5 as 5minutos, 
        carga15 as 15minutos 
    FROM grafana.sistema
\end{lstlisting}
